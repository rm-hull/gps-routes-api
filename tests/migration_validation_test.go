		...
		t.Run("ByActivity", func(t *testing.T) {
			...
			if err != nil {
				...
				for sqliteRows.Next() {
					var activity string
					var count int
					sqliteRows.Scan(&activity, &count)
...
func BenchmarkQueryPerformance(b *testing.B) {
	...
	b.Run("PostgreSQL_FullTextSearch", func(b *testing.B) {
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			pgPool.Query(context.Background(),
	...
	b.Run("SQLite_FullTextSearch_Placeholder", func(b *testing.B) {
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			sqliteDB.Query("SELECT COUNT(*) FROM routes_fts WHERE routes_fts MATCH 'hiking*'")
		}
	})
}
