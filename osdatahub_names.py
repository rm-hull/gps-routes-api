...
class OSDataHub:
    ...
    def nearby(self, lat: float, lng: float, radius: int = 1000):
        ...
        if "header" not in json_payload:
            print(f"PANIC! unexpected payload\n{resp.text}")
            exit(-1)

        if json_payload["header"]["totalresults"] != 1:
            return None

        # return (json_payload["results"][0]["GAZETTEER_ENTRY"],)
        return pick_attributes(
            json_payload["results"][0]["GAZETTEER_ENTRY"],
            keys=[
                "NAME1",
                "LOCAL_TYPE",
                "POPULATED_PLACE",
